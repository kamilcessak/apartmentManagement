...
export const DetailsFilesSection: FC<Props> = ({ files, id, type }) => {
  ...
  const tk = (key: string) => t(`rentals.rentalDetails.files.${key}`);

  const title = type === "photos" ? tk("photosTitle") : tk("documentsTitle");
  ...
  const handlePatchRental = async (file: string) => {
    ...
    const data =
      type === "documents"
        ...
        : { photos: resultFiles };
    return api.patch(`/rental/${id}`, data);
  };
  ...
  const {
    ...
    mutationFn: async (fileKey: string) => {
      const response = await api.get<{ url: string }>(`/upload/${fileKey}`);
      return {
  ...
  const uploadBusy = isFileUploading || isRentalPatching;
        ...
        <div className="flex flex-col gap-3">
          {fileList.map((fileKey) => (
            <RentalFileListRow
              ...
              isRemoving={isDeletingUpload && deleteTargetKey === fileKey}
